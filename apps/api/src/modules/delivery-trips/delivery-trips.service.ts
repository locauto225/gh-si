...
import type {
  DeliveryTripsListQuery,
  DeliveryTripCreateInput,
  DeliveryTripSetStatusInput,
  DeliveryStopCreateInput,
  StopSetStatusInput,
  StopPaymentCreateInput,
  StopDeliveriesAttachInput,
...
function makeTripNumber(seq: number) {
  // TRIP-2026-0001
  const n = String(seq).padStart(4, "0");
  return `TRIP-${year()}-${n}`;
}

async function nextTripSeqTx(tx: Prisma.TransactionClient) {
...
async function assertTripExists(id: string) {
  const item = await prisma.deliveryTrip.findUnique({
    where: { id },
    include: includeTripFull,
  });
  if (!item) {
    throw new AppError("Tournée introuvable", {
      status: 404,
...
async function assertStopExists(stopId: string) {
  const item = await prisma.deliveryStop.findUnique({
    where: { id: stopId },
    include: {
  ...
  if (!item) {
    throw new AppError("Arrêt introuvable", {
      status: 404,
      code: ERROR_CODES.NOT_FOUND,
      details: { stopId },
    });
...
export const deliveryTripsService = {
  list: async (q: DeliveryTripsListQuery) => {
    const take = Math.min(Math.max(Number(q.limit ?? 50) || 50, 1), 200);

    const where: any = {};
    if (q.status) where.status = q.status;
    if (q.fromWarehouseId) where.fromWarehouseId = q.fromWarehouseId;
    if (q.driverId) where.driverId = q.driverId;

    const items = await prisma.deliveryTrip.findMany({
      where,
      take,
  ...
  create: async (input: DeliveryTripCreateInput) => {
    const item = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (!wh) {
        throw new AppError("Entrepôt introuvable", {
          status: 404,
          code: ERROR_CODES.NOT_FOUND,
      ...
      if (input.driverId) {
        ...
        if (!dr) {
          throw new AppError("Livreur introuvable", {
            status: 404,
            code: ERROR_CODES.NOT_FOUND,
  ...
  setStatus: async (id: string, input: DeliveryTripSetStatusInput) => {
    ...
    const item = await prisma.deliveryTrip.update({
      where: { id },
      data,
      include: includeTripFull,
    });
  ...
  addStop: async (tripId: string, input: DeliveryStopCreateInput) => {
    ...
    const item = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (input.clientId) {
        ...
        if (!c) throw new AppError("Client introuvable", { status: 404, code: ERROR_CODES.NOT_FOUND });
      }
      if (input.storeId) {
        const s = await tx.store.findUnique({ where: { id: input.storeId }, select: { id: true } });
        if (!s) throw new AppError("Magasin introuvable", { status: 404, code: ERROR_CODES.NOT_FOUND });
      }
      ...
      if (uniqueSaleIds.length > 0) {
        ...
        const foundSet = new Set(found.map((x: { id: string }) => x.id));
        const missing = uniqueSaleIds.filter((x: string) => !foundSet.has(x));
        if (missing.length > 0) {
          throw new AppError("Ventes introuvables", {
            status: 404,
            code: ERROR_CODES.NOT_FOUND,
            details: { missing },
          });
        ...
        await tx.stopSale.createMany({
          data: uniqueSaleIds.map((saleId: string) => ({ stopId: stop.id, saleId })),
        });
      }
  ...
  setStopStatus: async (stopId: string, input: StopSetStatusInput) => {
    ...
    if (input.note !== undefined) data.note = input.note;
    if (input.visitedAt) data.visitedAt = input.visitedAt;
    else if (["VISITED", "PARTIAL", "DONE", "FAILED"].includes(input.status)) data.visitedAt = new Date();
  ...
  addStopPayment: async (stopId: string, input: StopPaymentCreateInput) => {
    const stop = await assertStopExists(stopId);

    await prisma.stopPayment.create({
      data: {
        stopId,
        method: input.method,
        amount: input.amount,
  ...
  addDeliveriesToStop: async (stopId: string, input: StopDeliveriesAttachInput) => {
    const stop = await assertStopExists(stopId);

    const deliveryIds = [...new Set(input.deliveryIds ?? [])];
    if (deliveryIds.length === 0) {
      throw new AppError("Aucun BL sélectionné", { status: 400, code: ERROR_CODES.VALIDATION_ERROR });
    }
    ...
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (missing.length > 0) {
        throw new AppError("Bons de livraison introuvables", {
          status: 404,
          code: ERROR_CODES.NOT_FOUND,
          details: { missing },
        });
      ...
      if (conflicts.length > 0) {
        throw new AppError("Certains BL sont déjà affectés", {
          status: 409,
