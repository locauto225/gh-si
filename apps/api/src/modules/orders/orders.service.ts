...
import type { OrderCreateBody, OrderQuoteBody, OrdersListQuery } from "./orders.schemas";
...
function computeTotals(lines: Array<{ qty: number; unitPrice: number }>, shippingFee: number) {
...
async function getEffectiveUnitPrices(args: {
  warehouseId: string;
  lines: Array<{ productId: string; qty: number; unitPrice?: number | undefined }>;
}) {
  ...
  return args.lines.map((l) => {
    if (l.unitPrice != null) return { productId: l.productId, qty: l.qty, unitPrice: l.unitPrice };
    const fromList = priceMap.get(l.productId);
    if (typeof fromList === "number") return { productId: l.productId, qty: l.qty, unitPrice: fromList };
    // fallback product.price
    const fallback = productPriceMap.get(l.productId);
    if (typeof fallback === "number") return { productId: l.productId, qty: l.qty, unitPrice: fallback };

    // should never happen, but safety
    throw new AppError("Pricing not found", {
      status: 409,
...
export const ordersService = {
  list: async (q: OrdersListQuery) => {
    ...
    const enriched = items.map((o) => {
      ...
      let logisticsStatus: "TO_PREPARE" | "IN_PROGRESS" | "PARTIAL" | "DONE" | "NO_BL" | "CANCELLED" = "TO_PREPARE";
      if (o.status === "CANCELLED") logisticsStatus = "CANCELLED";
      else if (deliveriesCount === 0) logisticsStatus = "NO_BL";
      else if (!hasRemaining && allDeliveriesDone) logisticsStatus = "DONE";
      else if (anyOutForDelivery) logisticsStatus = "IN_PROGRESS";
      else if (hasRemaining && (anyDelivered || anyPartial)) logisticsStatus = "PARTIAL";
      else logisticsStatus = "TO_PREPARE";
      ...
      if (o.status === "DRAFT") nextAction = "CONFIRM";
      else if (o.status === "CONFIRMED") nextAction = "PREPARE";
      else if (o.status === "PREPARED" && deliveriesCount === 0) nextAction = "CREATE_BL";
      else if (o.status === "PREPARED" && deliveriesCount > 0) nextAction = "DISPATCH";
      else if (o.status === "SHIPPED" && hasRemaining) nextAction = "DELIVER_REMAINDER";
      else if (o.status === "SHIPPED" && !hasRemaining && allDeliveriesDone) nextAction = "CLOSE_ORDER";
  ...
  quote: async (body: OrderQuoteBody) => {
  ...
  create: async (body: OrderCreateBody) => {
  ...
  setStatus: async (id: string, next: "CONFIRMED" | "PREPARED" | "SHIPPED" | "DELIVERED" | "CANCELLED") => {
    return prisma.$transaction(async (tx) => {
      ...
      if (next === "PREPARED") data.preparedAt = now;
