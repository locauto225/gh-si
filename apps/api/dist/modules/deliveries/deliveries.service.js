...
function normalizeStatusFilter(raw) {
    ...
    for (const p of parts) {
        if (!deliveries_schemas_1.DELIVERY_STATUSES.includes(p)) {
            throw new errors_1.AppError("Invalid status filter", {
                status: 400,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
                details: { status: p, allowed: [...deliveries_schemas_1.DELIVERY_STATUSES, "all"] },
            });
        }
        out.push(p);
    }
...
async function assertDeliveryExists(id) {
    const item = await prisma_1.prisma.delivery.findUnique({
        where: { id },
        include: {
    ...
    if (!item) {
        throw new errors_1.AppError("Delivery not found", { status: 404, code: errors_1.ERROR_CODES.NOT_FOUND });
    }
    return item;
}
...
async function applyDeliveryToSaleLinesTx(tx, deliveryId) {
    ...
    if (!sale) {
        throw new errors_1.AppError("Sale linked to delivery not found", {
            status: 409,
            code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            details: { deliveryId },
        });
    ...
    for (const dl of delivery.lines) {
        ...
        if (!sl) {
            throw new errors_1.AppError("SaleLine not found for delivery line", {
                status: 409,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
        ...
        if (newDelivered > sl.qty) {
            throw new errors_1.AppError("Delivered quantity exceeds ordered quantity", {
                status: 409,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
                details: {
                    saleLineId: sl.id,
...
async function applyDeliveryToOrderLinesTx(tx, deliveryId) {
    const delivery = await tx.delivery.findUnique({
        where: { id: deliveryId },
        include: {
    ...
    if (!order) {
        throw new errors_1.AppError("Order linked to delivery not found", {
            status: 409,
            code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            details: { deliveryId },
        });
    ...
    for (const it of delivery.items) {
        ...
        if (!ol) {
            throw new errors_1.AppError("OrderLine not found for delivery item", {
                status: 409,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
...
async function createEventTx(tx, deliveryId, input) {
...
exports.deliveriesService = {
    list: async (q) => {
        ...
        const items = await prisma_1.prisma.delivery.findMany({
            where,
    ...
    */
    create: async (data) => {
        ...
        if (data?.orderId && !data?.saleId) {
            ...
            if (order.status === "DRAFT" || order.status === "CANCELLED") {
                throw new errors_1.AppError("Invalid order status", {
                    status: 409,
                    code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            ...
            const items = (data.lines ?? []).map((l) => {
                ...
                if (!ol) {
                    throw new errors_1.AppError("OrderLine not found", {
                        status: 400,
                        code: errors_1.ERROR_CODES.VALIDATION_ERROR,
                ...
                if (!Number.isFinite(qtyDelivered) || qtyDelivered <= 0) {
                    throw new errors_1.AppError("qtyDelivered invalide", {
                        status: 400,
                        code: errors_1.ERROR_CODES.VALIDATION_ERROR,
                ...
                if (qtyDelivered > remaining) {
                    throw new errors_1.AppError("Quantity exceeds remaining to deliver", {
                        status: 409,
                        code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            ...
            if (!items.length) {
                throw new errors_1.AppError("Au moins une ligne requise", {
                    status: 400,
                    code: errors_1.ERROR_CODES.VALIDATION_ERROR,
                });
            }
            const item = await prisma_1.prisma.delivery.create({
                data: {
                    number,
                    status: "PREPARED",
        ...
        if (!data?.saleId) {
            ...
            if (!fromWarehouseId) {
                throw new errors_1.AppError("fromWarehouseId requis", {
                    status: 400,
                    code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            ...
            if (data.toStoreId) {
                ...
                if (!store) {
                    throw new errors_1.AppError("Store not found", {
                        status: 404,
            ...
            if (!toWarehouseId) {
                throw new errors_1.AppError("Destination requise (toStoreId ou toWarehouseId)", {
                    status: 400,
                    code: errors_1.ERROR_CODES.VALIDATION_ERROR,
            ...
            if (data.transferId) {
                ...
                if (tid) {
                    ...
                    if (!t) {
                        throw new errors_1.AppError("Transfer not found", {
                            status: 404,
                            code: errors_1.ERROR_CODES.NOT_FOUND,
            ...
            if (!transferId) {
                const transferLines = (data.lines ?? []).map((l) => ({
                    productId: String(l.productId).trim(),
                    qty: Number(l.qty),
                    note: l.note?.trim() || null,
                }));
                const purpose = (data.transferPurpose ? String(data.transferPurpose) : "INTERNAL_DELIVERY");
                const createdTransfer = await stock_service_1.stockService.createDeliveryTransferDraft({
                    fromWarehouseId,
                    toWarehouseId,
                    note: `BL ${number} (auto)`,
                    purpose,
        ...
        if (!SALE_STATUSES.includes(sale.status)) {
            throw new errors_1.AppError("Invalid sale status", {
                status: 409,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
        ...
        if (sale.status !== "POSTED") {
            throw new errors_1.AppError("Sale must be POSTED before creating a delivery", {
                status: 409,
                code: errors_1.ERROR_CODES.VALIDATION_ERROR,
        ...
        if (!transferId && sale.channel === "STORE" && sale.storeId) {
            ...
            const transferLines = data.lines.map((l) => {
                ...
                if (!sl) {
                    throw new errors_1.AppError("SaleLine not found", {
                        status: 400,
                        code: errors_1.ERROR_CODES.VALIDATION_ERROR,
    ...
    setStatus: async (id, input) => {
        ...
        const item = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (next === "OUT_FOR_DELIVERY" && !existing.dispatchedAt)
        ...
        const full = await prisma_1.prisma.delivery.findUnique({
            where: { id: item.id },
            include: {
                sale: { include: { client: true, warehouse: true, lines: { include: { product: true } } } },
                order: { include: { client: true, warehouse: true, lines: { include: { product: true } } } },
    ...
    assign: async (id, input) => {
        ...
        let tripId = rawTripId || null;
        let stopId = rawStopId || null;
        ...
        if (stopId) {
            ...
            if (!stop) {
                throw new errors_1.AppError("Stop not found", { status: 404, code: errors_1.ERROR_CODES.NOT_FOUND });
            }
            if (!tripId)
                tripId = stop.tripId;
            if (tripId && tripId !== stop.tripId) {
                throw new errors_1.AppError("Stop does not belong to trip", {
                    status: 409,
                    code: errors_1.ERROR_CODES.VALIDATION_ERROR,
        ...
        if (tripId) {
            const trip = await prisma_1.prisma.deliveryTrip.findUnique({
                where: { id: tripId },
                select: { id: true },
            });
            if (!trip) {
                throw new errors_1.AppError("Trip not found", { status: 404, code: errors_1.ERROR_CODES.NOT_FOUND });
            }
        }
        const item = await prisma_1.prisma.delivery.update({
            where: { id },
            data: {
                tripId,
                stopId,
    ...
    trackByToken: async (token) => {
        ...
        if (!item) {
            throw new errors_1.AppError("Tracking token not found", { status: 404, code: errors_1.ERROR_CODES.NOT_FOUND });
        }
        // réponse "safe"
        return {
            item: {
