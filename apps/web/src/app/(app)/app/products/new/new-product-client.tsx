...
type ProductSupplierInput = {
  supplierId: string;
  supplierSku?: string;
  lastUnitPrice?: number | null; // ✅ null = vide autorisé
};
...
export default function NewProductClient() {
  ...
  const [purchasePriceXof, setPurchasePriceXof] = useState<string>(""); // ✅ was "0"
  ...
  const lowestSupplierPrice = useMemo(() => {
    ...
    return Math.min(...prices.map((v) => Math.trunc(v)));
  }, [productSuppliers]);
  ...
  const isDirty = useMemo(() => {
    const hasSupplierRows =
      productSuppliers.length > 0 &&
      productSuppliers.some(
        (s) =>
          Boolean(s.supplierId) ||
          Boolean(s.supplierSku?.trim()) ||
          (typeof s.lastUnitPrice === "number" && Number.isFinite(s.lastUnitPrice) && s.lastUnitPrice > 0)
      );

    const hasPackagingRows =
      packagingRows.length > 0 &&
      packagingRows.some(
        (r) =>
          Boolean(r.name?.trim()) ||
          r.units !== 0 ||
          Boolean(r.barcode?.trim()) ||
          Boolean(r.grossWeightGr?.trim()) ||
          Boolean(r.tareWeightGr?.trim()) ||
          r.refIdx !== null
      );
    ...
    return (
      ...
      unit !== "bouteille" ||
      Boolean(weightValue.trim()) ||
      weightUnit !== "g" ||
      Boolean(volumeValue.trim()) ||
      volumeUnit !== "ml" ||
      Boolean(purchasePriceXof.trim()) ||
      !isActive ||
      hasSupplierRows ||
      hasPackagingRows
    );
  ...
  async function refreshCategories() {
    try {
      ...
      setCategories([]);
      setErr(getErrMsg(e, "Impossible de charger les catégories"));
    }
  }
  ...
  useEffect(() => {
    // Auto-sélectionne la seule sous-catégorie disponible (sans écraser un choix manuel)
    if (!categoryId) {
    ...
    const only = cat?.subcategories?.length === 1 ? cat.subcategories[0] : null;

    const categoryChanged = lastCategoryIdRef.current !== categoryId;
    if (categoryChanged) {
    ...
    // Si l’utilisateur a déjà choisi, on ne touche pas
    if (subCategoryIds.length > 0) return;

    setSubCategoryIds([only.id]);
  }, [categoryId, categories, subCategoryIds]);

  useEffect(() => {
    if (refPriceTouched) return;
    if (purchasePriceXof.trim() !== "") return;
    if (lowestSupplierPrice === null) return;

    setPurchasePriceXof(String(lowestSupplierPrice));
  }, [lowestSupplierPrice, purchasePriceXof, refPriceTouched]);
  ...
  async function handleDeleteSubcategory(catId: string, subId: string, subName: string) {
    ...
    try {
      ...
      await refreshCategories();
      setSubCategoryIds((prev) => prev.filter((id) => id !== subId));
    } catch (e: unknown) {
  ...
  function applySupplierPriceToAll(fromIdx: number) {
    const from = productSuppliers[fromIdx];
    const price = from?.lastUnitPrice;

    if (price === null || price === undefined) return;

    setProductSuppliers((prev) =>
      prev.map((row, i) => (i === fromIdx ? row : { ...row, lastUnitPrice: price }))
    );
  }
  ...
  function applyUnitWeightToEmptyPackagings() {
    ...
    setPackagingRows((prev) => {
      ...
      for (let idx = 0; idx < next.length; idx += 1) {
        const row = next[idx];
        const gross = computeExpectedGrossWeight(next, idx, unitWeightGr);
  ...
  function computeExpectedGrossWeight(
    rows: ColisageRow[],
    idx: number,
    ...
    const parentIdx = row.refIdx;
    const parent = rows[parentIdx];
    if (!parent) return undefined;

    const parentGross = toOptionalNonNegativeInt(parent.grossWeightGr);
    const effectiveParentGross =
      parentGross ?? computeExpectedGrossWeight(rows, parentIdx, unitWeightGr);
    if (effectiveParentGross === undefined) return undefined;

    return row.units * effectiveParentGross;
  }
  ...
  function syncAutoGrossWeights(rows: ColisageRow[]): ColisageRow[] {
    ...
    for (let idx = 0; idx < next.length; idx += 1) {
      const row = next[idx];
      if (!row.grossWeightAuto) continue;
      const gross = computeExpectedGrossWeight(next, idx, unitWeightGr);
      if (gross === undefined) continue;
      const grossStr = String(gross);
      if (row.grossWeightGr === grossStr) continue;
      next[idx] = { ...row, grossWeightGr: grossStr };
      changed = true;
    }
  ...
  function getGrossWeightWarning(row: ColisageRow, idx: number): string | null {
    ...
    if (grossGr === undefined) return null;

    const expected = computeExpectedGrossWeight(packagingRows, idx, unitWeightGr);
    if (!expected || expected <= 0) return null;

    const diff = Math.abs(grossGr - expected);
    const ratio = diff / expected;

    // "grosse incohérence" = écart >= 35% et au moins 100 g
  ...
  function fixGrossWeightAtIndex(idx: number) {
    ...
    setPackagingRows((prev) => {
      ...
      const expected = computeExpectedGrossWeight(prev, idx, unitWeightGr);
      if (expected === undefined) return prev;
      const next = [...prev];
      next[idx] = {
        ...next[idx],
        grossWeightGr: String(expected),
  ...
  useEffect(() => {
    setPackagingRows((prev) => syncAutoGrossWeights(prev));
  }, [weightValue, weightUnit]);

  // ✅ Recap pour la confirmation
  ...
  const selectedSubCategoryNames = useMemo(() => {
    ...
    const c = categories.find((x) => x.id === categoryId);
    const map = new Map((c?.subcategories ?? []).map((sc) => [sc.id, sc.name] as const));
    return subCategoryIds.map((id) => map.get(id)).filter((x): x is string => Boolean(x));
  }, [categoryId, categories, subCategoryIds]);

  const confirmWarnings = useMemo(() => {
    const warnings: string[] = [];

    if (!name.trim()) warnings.push("Désignation produit manquante");
    if (!sku.trim()) warnings.push("Référence produit manquante");

    if (!categoryId) warnings.push("Catégorie non renseignée (optionnel mais conseillé)");

    const hasSubcats = (currentCategory?.subcategories?.length ?? 0) > 0;
    if (hasSubcats && subCategoryIds.length === 0) {
      warnings.push("Aucune sous-catégorie sélectionnée (optionnel)");
    }

    const refPrice = lowestSupplierPrice ?? Math.trunc(Number(purchasePriceXof) || 0);
    if (!refPrice) warnings.push("Prix d'achat à 0 (optionnel)");

    const hasSuppliers = productSuppliers.some((s) => s.supplierId);
    if (!hasSuppliers) warnings.push("Aucun fournisseur renseigné (optionnel)");
    if (hasSupplierWithoutPrice) {
      warnings.push("Au moins un fournisseur sans prix (le prix de référence servira en secours)");
    }

    if (packagingRows.length === 0) {
      warnings.push("Aucun colisage ajouté (ok si vente uniquement à l’unité)");
    }
  ...
  function openConfirm() {
    ...
    if (!sku.trim() || !name.trim()) {
      setErr("Merci de renseigner au minimum : Désignation produit et Référence produit.");
      return;
    }
    setConfirmOpen(true);
  }
  ...
  async function onCreate() {
    // SKU manuel obligatoire + nom obligatoire
    ...
    try {
      await saveTask.run(async () => {
        const cleanPackagings = packagingRows.map((r, idx) => ({
        ...
        await apiPost("/products", {
          ...
          isActive,
          purchasePrice: lowestSupplierPrice ?? Math.trunc(Number(purchasePriceXof) || 0),
          subCategoryIds,
          ...
          suppliers: productSuppliers
            ...
            .map((s) => ({
              ...
              supplierSku: s.supplierSku?.trim() ? s.supplierSku.trim() : undefined, // ✅ vide => pas envoyé
              lastUnitPrice:
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20 px-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="grid gap-1.5">
                      ...
                      <input
                        ...
                        placeholder="Ex: GH-000123"
                        required
                        className="bg-muted/30 px-3 py-2 rounded-lg border text-sm font-mono"
                      />
                    </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="text-[11px] font-black text-muted-foreground hover:text-primary transition-colors"
                ...
                <button
                  ...
                  onClick={() =>
                    setProductSuppliers([
                      ...productSuppliers,
                      { supplierId: "", supplierSku: "", lastUnitPrice: null }, // ✅ vide
                    ])
                  }
              ...
              {productSuppliers.length === 0 ? (
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody className="divide-y">
                    {productSuppliers.map((s, idx) => (
                      <tr key={idx} className="group">
                        <td className="p-2 px-4">
                          <select
                            value={s.supplierId}
                            onChange={(e) => {
                              const next = [...productSuppliers];
                              next[idx].supplierId = e.target.value;
                              setProductSuppliers(next);
                            }}
                        ...
                        <td className="p-2 text-right">
                          <button
                            onClick={() =>
                              setProductSuppliers(productSuppliers.filter((_, i) => i !== idx))
                            }
                    ...
                    <div className="flex items-center gap-2 bg-primary/5 px-3 py-2 rounded-xl border border-primary/20">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const next = [...packagingRows];
                          next[idx].refIdx =
                            e.target.value === "__base__" ? null : Number(e.target.value);
                          setPackagingRows(syncAutoGrossWeights(next));
                        }}
                    ...
                    <div className="flex-1 flex flex-wrap items-end justify-between gap-4 md:gap-6 md:flex-nowrap">
                      ...
                      <div className="hidden md:flex items-end gap-4 ml-auto shrink-0">
                        ...
                        <button
                          onClick={() =>
                            setPackagingRows((prev) =>
                              syncAutoGrossWeights(prev.filter((_, i) => i !== idx))
                            )
                          }
        ...
        <div className="space-y-6">
          <section className="bg-card border rounded-2xl p-6 shadow-sm space-y-6">
                  ...
                  {lowestSupplierPrice === null ? (
                    <>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setRefPriceTouched(true);
                          setPurchasePriceXof(e.target.value);
                        }}
                        placeholder="0"
            ...
            <div className="pt-6 border-t space-y-4">
              ...
              {/* Catégorie — select + création inline */}
              {isAddingCat ? (
              ...
              {/* Sous-catégories — chips + création inline (amélioré) */}
              {categoryId && (
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const ok = confirm(
                      next
                        ? "Confirmer l'activation du produit ?"
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-[min(720px,calc(100vw-32px))] rounded-2xl bg-card border shadow-xl">
            <div className="p-6 border-b">
              <div className="text-sm font-bold text-foreground">Vérification avant enregistrement</div>
              <div className="text-xs text-muted-foreground mt-1">
                Vérifie les infos ci-dessous pour éviter les oublis.
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Désignation</div>
                  <div className="mt-1 text-sm font-semibold text-foreground break-words">{name.trim() || "—"}</div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Référence produit</div>
                  <div className="mt-1 text-sm font-mono font-semibold text-foreground break-words">{sku.trim() || "—"}</div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Catégorie</div>
                  <div className="mt-1 text-sm font-semibold text-foreground break-words">{selectedCategoryName}</div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Sous-catégories</div>
                  <div className="mt-1 text-sm text-foreground break-words">
                    {selectedSubCategoryNames.length ? selectedSubCategoryNames.join(", ") : "—"}
                  </div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Unité de base</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">{unit}</div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Prix d'achat</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">
                    {lowestSupplierPrice ?? Math.trunc(Number(purchasePriceXof) || 0)} FCFA
                  </div>
                  <div className="mt-1 text-[10px] uppercase tracking-wide text-muted-foreground">
                    {lowestSupplierPrice !== null ? "Meilleur prix fournisseur" : "Prix de référence"}
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Fournisseurs</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">
                    {productSuppliers.filter((s) => s.supplierId).length}
                  </div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Colisages</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">{packagingRows.length}</div>
                </div>
                <div className="rounded-xl border bg-muted/10 p-4">
                  <div className="text-[10px] font-black text-muted-foreground uppercase">Actif</div>
                  <div className="mt-1 text-sm font-semibold text-foreground">{isActive ? "Oui" : "Non"}</div>
                </div>
              </div>

              {confirmWarnings.length > 0 && (
                <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4 text-amber-900">
                  <div className="text-xs font-black uppercase">À vérifier</div>
            ...
            <div className="p-6 border-t flex items-center justify-end gap-3">
              <button
                ...
                disabled={saving}
                className="px-4 py-2 rounded-xl border border-border text-sm font-semibold text-muted-foreground hover:text-foreground disabled:opacity-50"
              >
                Modifier
              </button>
              <button
                ...
                disabled={saving}
                className="bg-primary text-primary-foreground px-6 py-2 rounded-xl font-bold hover:shadow-lg hover:shadow-primary/20 disabled:opacity-50 transition-all"
              >
